...
int nHincoinLastNBlockHeight = 0;
int nHincoinLastRBlockHeight = 0;
...
const string strMessageMagic = "hincoin Signed Message:\n";
...
{
    // hincoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned char GetNfactor(int64 nTimestamp,const CBlockIndex* pindexLast2) {
   ...
   //     nHincoinLastUpdateTime = nTimestamp;
        ...
        nHincoinLastNBlockHeight = pindexLast->nHeight;
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // hincoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if(!nHincoinUsingRUpdate && pindexLast->nTime >= nHincoinStartRLearning)
    {
        ...
        if (fTestNet) {
        	...
        	if (pindexLast->nHeight+1 >= 26754) { DiffMode = 2; }  //hincoin, 5 days after 27/01/2014 12:00 UTC
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // hincoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // hincoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[2] = 'r';
        pchMessageStart[3] = 't';
        hashGenesisBlock = uint256("0xb91cda2749635612c51145f68f26494be57ac184f6d8040014ea3016cd4a8607");
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "03/01/14 Ukraine and Venezuela are in deep turmoil due to corrupt governments.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce = 1695989;
        block.nTime = 1393733693;
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // hincoin: increase each by adding 1 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("hincoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("hincoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static hincoinMiner(CWallet *pwallet)
{
    printf("hincoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hincoin-miner");
    ...
    try { loop {
        ...
        printf("Running hincoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("hincoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&hincoinMiner, pwallet));
}
